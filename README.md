...
$$ \boldsymbol{ \chi}_f^*= \arg \min_{ \chi_f} \left \\{ \sum_{i=k-n+1}^{k-1} \left \\|(z_i^O-h^O( \mathbf{T}_i, \mathbf{T}_{i+1})) \right \\|_{ \Omega_i^O}+ \sum_{j=k-m+1}^k \left \\|(z_j^M-h^M( \mathbf{T}_j)) \right \\|_{ \Omega_j^M} \right \\} $$
