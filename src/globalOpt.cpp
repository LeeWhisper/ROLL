...
GlobalOptimization::GlobalOptimization(int maxNo)//500
{
	initGPS = false;
    newGPS = false;
    newGlobalLocPose = false;
	WGlobal_T_WLocal = Eigen::Matrix4d::Identity();//单位矩阵
    threadOpt = std::thread(&GlobalOptimization::optimize, this);//多线程
    maxFrameNum = maxNo;
...
void GlobalOptimization::inputOdom(double t, Eigen::Matrix4d affine)// msgIn->header.stamp.toSec(),affine_imu_to_odom_tmp.matrix().cast<double>()
{
...
void GlobalOptimization::optimize()//线程函数，不断检查是否有新的全局定位信息，有则进行全局优化，更新全局位姿
{
    while(true)
    {
        if(newGlobalLocPose)
        {
            ...
            parameters.relinearizeSkip = 1;// 重线性化跳过步数
            ...
            for (iterLIO = localPoseMap.begin(); iterLIO != localPoseMap.end(); iterLIO++, i++,iterIni++)
            {
                ...
                if(iterLIOnext != localPoseMap.end())
                {
                    ...
                    gtSAMgraphTM.add(BetweenFactor<Pose3>(i,i+1, poseFrom.between(poseTo), odometryNoise));//添加里程计因子
                }
                ...
                if (iterGlobalLoc != globalLocPoseMap.end())
                {
                    ...
                    gtSAMgraphTM.add(PriorFactor<Pose3>(i, poseGlobal, corrNoise));//添加全局定位因子
                    found++;
            ...
            for (int i = 0; i < length; i++, iterIni++)// 遍历局部位姿图，更新全局位姿
            {
